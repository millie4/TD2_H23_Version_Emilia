...
void ajouterFilm(Film* film, ListeFilms& liste){
	Film** nouveauxElements;
	if (liste.nElements == liste.capacite){
		// Créer un nouveau tableau d'une plus grande capacité
		if (liste.capacite == 0){
			liste.capacite = 1;
			nouveauxElements = new Film* [1];
		}
		else{
			nouveauxElements = new Film* [liste.capacite * 2];
			liste.capacite *= 2;
			for (int i = 0; i < liste.nElements; i++) {
				nouveauxElements[i] = liste.elements[i];
			}
		}
		// Copier les valeurs de l'ancienne liste
...
void enleverFilm(Film* film, ListeFilms& liste){
	liste.elements[];
}

//TODO: Une fonction pour trouver un Acteur par son nom dans une ListeFilms, qui retourne un pointeur vers l'acteur, ou nullptr si l'acteur n'est pas trouvé.  Devrait utiliser span.
Acteur* trouverActeur(ListeFilms liste, string nomActeur){
	Acteur* ptrActeur = new Acteur;
	span<Film*> tableauFilms = span(liste.elements);
	for (Film* film : tableauFilms) {
		span<Acteur*> tableauActeurs = span(film->acteurs.elements);
		for (Acteur* acteur : tableauActeurs) {
